...
import { ShieldCheck, AlertTriangle, Lock, ArrowRight, Eye, Radio, CalendarClock } from 'lucide-react';
...
export default function MissionCard({
  ...
  return (
    <div
      ...
      className={`group relative p-4 sm:p-5 rounded-2xl border transition-all select-none flex flex-col justify-between text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-cyber-cyan ${
        isLocked
          ? 'bg-cyber-950/40 border-slate-800/60 opacity-60 cursor-not-allowed'
          : isCompleted
    ...
    >
      ...
      <div>
        <div className="flex items-center justify-between gap-2 mb-3">
          ...
          {/* Status Badge */}
          {isLocked ? (
            <span className="inline-flex items-center gap-1 text-[10px] font-mono font-bold text-slate-400 bg-slate-900 px-2 py-0.5 rounded border border-slate-700">
              <CalendarClock className="w-3 h-3" />
              {unlockDateLabel ? `UNLOCKS ${unlockDateLabel.toUpperCase()}` : 'LOCKED'}
            </span>
          ) : isCompleted ? (
        ...
        <h4 className="text-sm sm:text-base font-cyber font-bold text-white mb-1 group-hover:text-cyber-cyan transition-colors leading-snug">
          {isLocked ? '??? CLASSIFIED UNTIL UNLOCK ???' : mission.title}
        </h4>
        <p className="text-xs font-sans text-slate-400 line-clamp-2 mb-4 leading-relaxed">
          {isLocked ? `This mission unlocks ${unlockDateLabel}.` : mission.topic}
        </p>
      </div>
      ...
      <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between text-[11px] font-mono">
        <span className="text-slate-500 group-hover:text-slate-300">
          {isLocked ? 'Status: Pending' : `Source: ${mission.alertSource}`}
        </span>
        {!isLocked && (
